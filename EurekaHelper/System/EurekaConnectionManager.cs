...
namespace EurekaHelper.System
{
    public class EurekaConnectionManager : IDisposable
    {
        ...
        public static async Task<EurekaConnectionManager> Connect()
        {
            ...
            try
            {
                await connection.ClientWebSocket.ConnectAsync(
                    new Uri(TrackerUrl),
            ...
            do
            {
                ...
                switch (message.Event)
                {
                    ...
                    case "presence_diff":
                        Viewers =
                            Viewers
                            + ((JObject)message.Payload["joins"]).Count
                            - ((JObject)message.Payload["leaves"]).Count;
                        break;
                    ...
                    case "phx_reply":
                        ...
                        if (!((string)message.Payload["status"]).Equals("ok"))
                        {
                            ...
                            {
                                if (
                                    ((string)message.Payload["response"]).Equals(
                                        "Instance does not exist"
                                    )
                                )
                                {
                                    DalamudApi.Log.Information(
                                        "Invalid instance. Closing connection"
                                    );
                            ...
                            DalamudApi.Log.Information(
                                $"Received status: \"{message.Payload["status"]}\" and response: \"{message.Payload["response"]["reason"]}\". Closing connection"
                            );
                            await Close();
                            break;
                        }
                    ...
                    case "initial_payload":
                        ...
                        int zoneId = (int)
                            message.Payload["data"]["relationships"]["zone"]["data"]["id"];
                        Tracker = Utils.GetEurekaTracker((ushort)zoneId);

                        TrackerId = (string)message.Payload["data"]["id"];
                        TrackerPassword =
                            message.Payload["data"]["attributes"]["password"] != null
                                ? (string)message.Payload["data"]["attributes"]["password"]
                                : String.Empty;
                        Public =
                            message.Payload["data"]["attributes"]["data-center-id"].Type
                            != JTokenType.Null;
                        ...
                        {
                            var notoriousMonsters = JObject.Parse(
                                (string)message.Payload["data"]["attributes"]["notorious-monsters"]
                            );
                    ...
                    case "payload":
                        ...
                        {
                            if (
                                message.Payload["data"]["attributes"]?["notorious-monsters"] != null
                            )
                            {
                                var notoriousMonsters = JObject.Parse(
                                    (string)
                                        message.Payload["data"]["attributes"]["notorious-monsters"]
                                );
                                Dictionary<ushort, long> keyValuePairs = new();
                                foreach (var monster in notoriousMonsters)
                                    keyValuePairs.Add(
                                        ushort.Parse(monster.Key),
                                        (long)monster.Value
                                    );
        ...
        public static async Task<EurekaConnectionManager> JoinTracker(
            string trackerId,
        ...
        public async Task SendHeartbeat()
        {
            ...
            {
                DalamudApi.Log.Information(
                    "No response to heartbeat message received within 30seconds. Closing connection"
                );
                await Close();
                return;
            }
        ...
        public async Task SetTrackerVisiblity(int dataCenterId = -1)
        {
            EurekaTrackerMessage eurekaTrackerMessage = new(
                ...
                JObject.Parse(
                    $"{{ instance_id: null, data_center_id: {(dataCenterId == -1 ? "null" : dataCenterId)} }}"
                )
            );
        ...
        {
            string jsonContent = JObject
                .Parse(
                    @$"{{ 'data': {{ 'attributes': {{ 'zone-id': {zoneId} }}, 'type': 'instances' }} }}"
                )
                .ToString();
        ...
        public static async Task<(string trackerId, string password)> ExportTracker(
            string oldTrackerId
        )
        {
            string jsonContent = JObject
                .Parse(
                    @$"{{ 'data': {{ 'attributes': {{ 'copy-from': '{oldTrackerId}' }}, 'type': 'instances' }} }}"
                )
                .ToString();
